...
Java_com_airobot_agent_audio_tools_codec_OpusDecoder_nativeInitDecoder(
...
Java_com_airobot_agent_audio_tools_codec_OpusDecoder_nativeDecodeBytes(
        JNIEnv *env, jobject thiz,
    ...
    return result * 2; // Returns number of bytes (2 bytes per sample)
}

JNIEXPORT void JNICALL
Java_com_airobot_agent_audio_tools_codec_OpusDecoder_nativeReleaseDecoder(
        JNIEnv *env, jobject thiz,
        jlong decoder_handle) {
    OpusDecoder *decoder = (OpusDecoder*)(intptr_t)decoder_handle;
    if (decoder != nullptr) {

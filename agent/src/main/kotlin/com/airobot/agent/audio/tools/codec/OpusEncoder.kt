package com.airobot.agent.audio.tools.codec
...
class OpusEncoder(
    ...
    suspend fun encode(pcmData: ByteArray): ByteArray? = withContext(Dispatchers.IO) {
        ...
        mutex.withLock {
            val outputBuffer = ByteArray(frameBytes) // Allocate a large enough buffer
    ...
    suspend fun reset() = withContext(Dispatchers.IO) {
        mutex.withLock {
            Log.d(TAG, "Resetting Opus encoder state...")
            release()
            nativeEncoderHandle = nativeInitEncoder(sampleRate, channels, 2048) // OPUS_APPLICATION_VOIP
            if (nativeEncoderHandle == 0L) {
                Log.e(TAG, "Failed to re-initialize Opus encoder during reset")
            }
        }
    }

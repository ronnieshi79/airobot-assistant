...
Java_com_airobot_agent_audio_tools_codec_OpusEncoder_nativeInitEncoder(
    ...
    opus_encoder_ctl(encoder, OPUS_SET_COMPLEXITY(10));  // 0-10, 10 is the highest quality
...
Java_com_airobot_agent_audio_tools_codec_OpusEncoder_nativeEncodeBytes(
        JNIEnv *env, jobject thiz,
...
Java_com_airobot_agent_audio_tools_codec_OpusEncoder_nativeReleaseEncoder(
        JNIEnv *env, jobject thiz,
        jlong encoder_handle) {
    OpusEncoder *encoder = (OpusEncoder *) (intptr_t) encoder_handle;
    if (encoder != nullptr) {
